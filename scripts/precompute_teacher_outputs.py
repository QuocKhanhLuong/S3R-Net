...
def main() -> None:
    ...
    for batch in tqdm(loader, desc="precompute teachers"):
        ...
        payload: dict[str, Any] = {
            "P_M3": out_m3["probs"][0].detach().cpu(),
            "C_M3": out_m3["confidence"][0].detach().cpu(),
            "P_C": out_c["probs"][0].detach().cpu(),
            "C_C": out_c["confidence"][0].detach().cpu(),
            "B_C": out_c.get("boundary", torch.zeros_like(out_c["confidence"]))[0].detach().cpu(),
            "metadata": {
                "case_id": case_id,
                "slice_idx": slice_idx,
                "shape": list(out_m3["probs"].shape[-2:]),
